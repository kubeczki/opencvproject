...
def system_sampling(sampling_step, file_name, resolution):
    raw_input_queue = mp.Queue(1)
    detection_queue = mp.Queue(1)
    faces_queue = mp.Queue(1)
    time_control_queue = mp.Queue(2)

    producer = mp.Process(target=produce_data_samp, args=(raw_input_queue, detection_queue, time_control_queue, file_name, resolution, ))
    producer.start()
